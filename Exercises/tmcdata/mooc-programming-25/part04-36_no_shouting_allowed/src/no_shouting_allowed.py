...
def no_shouting(my_list: list):
    ...
    for word in my_list:
        if word.isupper() == False:
            no_allcaps_list.append(word)

    return no_allcaps_list
...
#Review
My solutions results in the same output, the suggested one uses the "if not" as a
conditional statement which is the only difference.
"""
