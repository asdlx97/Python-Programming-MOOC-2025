...
def accepted(attempts: list):
    return filter(lambda c: c.grade >= 1, attempts)

def attempts_with_grade(attempts: list, grade: int):
    return filter(lambda c: c.grade == grade, attempts)

def passed_students(attempts: list, course: str):
    filtered = filter(lambda s: s.grade > 0 and s.course_name == course, attempts)
    return sorted(list(map(lambda c: c.student_name, filtered)))

if __name__ == "__main__":
    #Part1: Accepted attempts
    s1 = CourseAttempt("Peter Python", "Introduction to Programming", 3)
    s2 = CourseAttempt("Olivia C. Objective", "Introduction to Programming", 5)
    s3 = CourseAttempt("Peter Python", "Advanced Course in Programming", 0)
    ...
    #Part3: Students who passed the course
    s1 = CourseAttempt("Peter Python", "Introduction to Programming", 3)
    s2 = CourseAttempt("Olivia C. Objective", "Introduction to AI", 5)
    s3 = CourseAttempt("Peter Python", "Introduction to AI", 0)
    s4 = CourseAttempt("Jack Java", "Introduction to AI", 3)
...
#Review
My solution results in the same output, the suggested one doesn't convert
to a list before sorting the iteration that results from filter() and map().
"""