...
class PaymentTerminal:
    ...
    def eat_special(self, payment: float):
        ...
        price = 4.30
        if payment >= price:
            # Increase the value of the funds at the terminal by the price of the special,
            self.funds += price
    ...
    def eat_special_lunchcard(self, card: LunchCard):
        # A special costs 4.30 euros.
        price = 4.30
        # If there is enough money on the card, subtract the price of the special from the balance
        # and return True. If not, return False.
        if card.balance >= price:
            self.specials += 1
            card.balance -= price
...
#Review
My solution results in the same output, suggested 
one uses given subtract_from_balance() method from the
LunchCard class, which makes sense.
"""
