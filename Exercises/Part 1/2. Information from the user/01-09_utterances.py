# Fix the code: Utterances

# Here is a program which should ask for three utterances and print them out, like so:

# Sample output
# The 1st part: hickory
# The 2nd part: dickory
# The 3rd part: dock
# hickory-dickory-dock!

# However, there is something wrong with the code below. Please fix it.
# Fix the code
# part = input("The 1st part: ")
# part = input("The 1st part: ")
# part = input("The 1st part: ")
# print(part + part + part)

part1 = input("The 1st part: ") #Named variables separately
part2 = input("The 2st part: ") #Updated printed input text per variable
part3 = input("The 3st part: ")
print(part1 + "-" + part2 + "-" + part3 + "!") #Made sure print is formatted properly using concat with hyphens and excl mark