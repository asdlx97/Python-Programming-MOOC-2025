...
def sort_by_number_of_routes(areas: list):
    #Helper function
    def by_n_routes(item: ClimbingArea):
        return item.routes()

    return sorted(areas, key=by_n_routes)

def sort_by_most_difficult(areas: list):
    #Helper function
    def by_hardest(item: ClimbingArea):
        return item.hardest_route().grade #This .grade was needed otherwise it used the Climbing Route object later on
