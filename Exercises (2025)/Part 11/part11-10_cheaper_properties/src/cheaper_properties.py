...
def cheaper_properties(properties: list, reference: RealProperty):
    return [(property, property.price_difference(reference)) for property in properties if property.price_difference(reference) > 0 and not property.more_expensive(reference)] #Had to add not property.more_expensive check because price_difference return absolute values and furthermore, more_expensive on its own doesn't work as it returns values equal to 0
...
#Review
My solution results in the same output, the suggested one however uses the reference
as a basis to call the more_expensive function on which makes more sense as I had to
add a price_difference method besides my if not more_expensive() method with the current list item as basis
